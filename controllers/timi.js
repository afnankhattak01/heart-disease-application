...
const TimiController = async (req, res) => {
  ...
  try {
    const resp = await axios.post(
      "https://sdlc4ml.ixxhar.com/MedicalAnalysis/TimiNStemiCalculator",
      req.body
    );
    if (resp.status === 200 && req.user._id) {
      try {
        ...
        if (user) {
          try {
            const hDraft = await TimiSchema.create({
              ...req.body,
              user: req.user._id,
              result: resp.data,
            });
            ...
            return res.status(200).json({
              success: true,
              message: "Successfully calculated Timi Risk!",
              data: {
